...
module CoreUnfold (
        ...
        computeDiscount, ExprSize(..), sizeExpr, -- Changed
...
calcUnfoldingGuidance dflags is_top_bottoming expr
  = case sizeExpr dflags bOMB_OUT_SIZE val_bndrs body of
      ...
      SizeIs size cased_bndrs scrut_discount
        | uncondInline expr n_val_bndrs size dflags
...
uncondInline :: CoreExpr -> Arity -> Int -> DynFlags  -> Bool -- Note: TODO pass DynFlags
-- Inline unconditionally if there no size increase
-- Size of call is arity (+1 for the function)
-- See Note [INLINE for small functions]
uncondInline rhs arity size dflags -- Celetse: TODO add dflags
  | arity > 0 = size <= (uoeUncInl dflags) * (arity + 1) -- See Note [INLINE for small functions] (1) -- Changes: default 10 
...
sizeExpr dflags bOMB_OUT_SIZE top_args expr
  ...
  where
    ...
    size_up (Lit lit)  = sizeN (litSize lit dflags) -- Changes: pass dflags
    size_up (Var f) | isRealWorldId f = sizeZero
                      -- Make sure we get constructor discounts even
                      -- on nullary constructors
                    | otherwise       = size_up_call dflags f [] 0 -- Note: added dflags
    ...
    size_up (Case e _ _ alts)
        ...
        = let
            ...
            alts_size (SizeIs tot tot_disc tot_scrut)
                  ...
                  = SizeIs tot (unitBag (v, (uoeBigAlt dflags) + tot - max) -- Changes : default 20
    ...
    size_up_app (Var fun)     args voids = size_up_call dflags fun args voids -- Note: add dflags
    ...
    size_up_app other         args voids = size_up other `addSizeN`
                                           callSize (length args) dflags voids -- Note: add dflags
       -- if the lhs is not an App or a Var, or an invisible thing like a
       -- Tick or Cast, then we should charge for a complete call plus the
       -- size of the lhs itself.

    ------------
    size_up_call :: DynFlags -> Id -> [CoreExpr] -> Int -> ExprSize -- Note: pass DynFlags
    size_up_call dflags fun val_args voids -- Note: add dflags
       = case idDetails fun of
           FCallId _        -> sizeN (callSize (length val_args) dflags voids) -- Note add dflags
           DataConWorkId dc -> conSize    dc (length val_args) dflags
...
litSize :: Literal -> DynFlags -> Int
-- Used by CoreUnfold.sizeExpr
litSize (LitNumber LitNumInteger _ _) dflags = 100   -- Note [Size of literal integers]
litSize (LitNumber LitNumNatural _ _) dflags = 100
litSize (LitString str) dflags = (uoeLitStr dflags) + 10 * ((BS.length str + 3) `div` 4)  -- Changes: default 10
        -- If size could be 0 then @f "x"@ might be too small
        -- [Sept03: make literal strings a bit bigger to avoid fruitless
        --  duplication of little strings]
litSize _other dflags = 0    -- Must match size of nullary constructors
...
classOpSize dflags top_args (arg1 : other_args)
  ...
  where
    size = (uoeCOSbase dflags) + ((uoeCOSargs dflags) * length other_args) -- Changes: uoeCOSbase defaults 20, uoeCOSargs default 10
...
callSize
 ...
 -> Int
callSize n_val_args dflags voids = 10 * (1 + n_val_args - voids) -- Changes: uoeCallSize default 10 (removed flag & put default back)
...
funSize dflags top_args fun n_val_args voids
  ...
  where
    ...
    size | is_join              = jumpSize n_val_args voids
         | not some_val_args    = 0
         | otherwise            = callSize n_val_args dflags voids -- Note: add dflags
...
conSize :: DataCon -> Int -> DynFlags -> ExprSize -- Changes: Added DynFlags
conSize dc n_val_args dflags -- Changes: pass dflags
...
-- See Note [Constructor size and result discount]
  | otherwise = SizeIs (uoeConSize dflags) emptyBag (10 * (1 + n_val_args)) -- Changes: default 10
...
computeDiscount dflags arg_discounts res_discount arg_infos cont_info
  ...
  = (uoeFuncItself dflags)          -- Discount of 10 because the result replaces the call -- Note: default 10
                -- so we count 10 for the function itself

    + (uoeActArg dflags) * length actual_arg_discounts -- Note: default 10
  ...
  where
    ...
    mk_arg_discount _        NonTrivArg = uoeNonTrivArg dflags -- 10 -- Note
    ...
    res_discount'
      ...
      = case cont_info of
           ...
           -- Changes: added the DiscArgCtxt, RuleArgCtxt, and RhsCtxt cases
           -- per the comments below in ToDo to see what happens
           DiscArgCtxt -> uoeDiscArgCtxt dflags `min` res_discount -- Changes: added this
           RuleArgCtxt -> uoeRuleArgCtxt dflags `min` res_discount -- Changes: added this
           RhsCtxt     -> uoeRhsCtxt dflags `min` res_discount  -- Changes: added this
           _           -> (uoeArbCtxt dflags) `min` res_discount
