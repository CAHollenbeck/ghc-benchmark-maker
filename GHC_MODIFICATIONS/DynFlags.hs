...
data DynFlags = DynFlags {
  -- begin Changes
  -- These are the threshold flags for randomizing
  -- the magic numbers in coreSyn/CoreUnfold.hs
   uoeNonTrivArg         :: Int,
             -- Discount for an argument labeled "NonTrivial". Default 10.
   uoeFuncItself         :: Int,
             -- uoeFuncItself : Constant discount amount added to every function inlined.
             -- Default 10.
   uoeActArg             :: Int,
             -- uoeActArg : Multiply the number of arguments by this number
             -- to scale the discount. More arguments means bigger discount.
             -- Default 10.
   uoeDiscArgCtxt        :: Int,
             -- "Argument of a function with non-zero arg discount". Default 40.
             -- Originally in a catch-all discount of 40.
   uoeRuleArgCtxt        :: Int,
             -- "We are somewhere in the argument of a function with rules"
             -- Originally in a catch-all discount of 40.
             -- coreSyn/Coreunfold.hs
   uoeRhsCtxt            :: Int,
             -- RHS of a let. Note in coreSyn/CoreUnfold.hs
             -- Originally in a catch-all discount of 40.
   uoeArbCtxt            :: Int,
             -- The wild card ( _ ) remaining to catch any other types of 
             -- cont_info and calculate its discount
             -- This is what remains of the catch-all discount of 40.
   uoeCOSbase            :: Int,
             -- Base size value of a class op. Default 20.
   uoeCOSargs            :: Int,
             -- Size metric added to each argument of a class op. Default 10.
   uoeBigAlt             :: Int,
             -- Discount metric for the biggest alternative when scrutinizing an
             -- arument variable in a case expression. Default 20.
   uoeConSize            :: Int,
             -- Metric for the size of a constructor
   uoeLitStr             :: Int,
             -- Size of literal string
   uoeUncInl             :: Int,
             -- Size for determining when to unconditionally inline, assuming arity > 0
   -- end Changes
  ghcMode               :: GhcMode,
  ghcLink               :: GhcLink,
  hscTarget             :: HscTarget,
...
-- BEGIN Changes
  , make_ord_flag defFlag "thresh-funcitself-disc"
      (intSuffix (\n d -> d { uoeFuncItself = n }))
  , make_ord_flag defFlag "thresh-actarg-disc"
      (intSuffix (\n d -> d { uoeActArg = n }))
  , make_ord_flag defFlag "thresh-nontrivarg-disc"
      (intSuffix (\n d -> d { uoeNonTrivArg = n }))
  , make_ord_flag defFlag "thresh-discargctxt-disc"
      (intSuffix (\n d -> d { uoeDiscArgCtxt = n }))
  , make_ord_flag defFlag "thresh-ruleargctxt-disc"
      (intSuffix (\n d -> d { uoeRuleArgCtxt = n }))
  , make_ord_flag defFlag "thresh-rhsctxt-disc"
      (intSuffix (\n d -> d { uoeRhsCtxt = n }))
  , make_ord_flag defFlag "thresh-arbctxt-disc"
      (intSuffix (\n d -> d { uoeArbCtxt = n }))
  , make_ord_flag defFlag "thresh-cosbase"
      (intSuffix (\n d -> d { uoeCOSbase = n }))
  , make_ord_flag defFlag "thresh-cosargs"
      (intSuffix (\n d -> d { uoeCOSargs = n }))
  , make_ord_flag defFlag "thresh-bigalt"
      (intSuffix (\n d -> d { uoeBigAlt = n }))
  , make_ord_flag defFlag "thresh-consize"
      (intSuffix (\n d -> d { uoeConSize = n }))
  , make_ord_flag defFlag "thresh-litstr"
      (intSuffix (\n d -> d { uoeLitStr = n }))
  , make_ord_flag defFlag "thresh-uncinl"
      (intSuffix (\n d -> d { uoeUncInl = n }))
-- END Changes
