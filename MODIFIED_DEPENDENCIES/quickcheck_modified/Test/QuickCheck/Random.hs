...
import Debug.Trace -- Celeste: to tag the test output and make sure QC is deterministic.
...
newQCGen :: IO QCGen
#ifdef NO_SPLITMIX
newQCGen = trace "DETERMINISTIC" (return (mkQCGen 12345)) -- Celeste: original: fmap QCGen newStdGen
#else
newQCGen = trace "DETERMINISTIC" (return (mkQCGen 12345)) -- Celeste: original: fmap QCGen newSMGen
#endif

mkQCGen :: Int -> QCGen
#ifdef NO_SPLITMIX
mkQCGen n = QCGen (mkStdGen 12345)-- n)
#else
mkQCGen n = QCGen (mkSMGen (fromIntegral 12345))-- n))
#endif
